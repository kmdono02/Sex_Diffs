...
# Need to remove LR kids without V24
# First extract ids of LR kids with V24 using long-form data
lr_ids_tokeep <- dataset_long %>% 
  filter(Risk_dummy=="LR"&Visit=="V24"&is.na(V24_Group)==0) %>% 
  select(Identifiers) %>% 
  unlist()

# Need to replace values with labels for certain variables
dataset_wide_forsum_stats <- 
  dataset_wide %>% filter(Identifiers%in%soc_factor_data$Identifiers|Identifiers%in%lr_ids_tokeep)
...
# Also add in additional demo info
added_demo_info <- read_csv("Data/TSI_V06.csv")
...
dataset_wide_forsum_stats_v2 <- left_join(dataset_wide_forsum_stats, added_demo_info) %>%
  mutate(`V06 tsi,mother_education_edited`=gsub("hs", "high school",
                                                firstup(gsub("_"," ",`V06 tsi,mother_education`))),
         `V06 tsi,ses_income_edited`=gsub("Not A", "Not a",
                                          gsub("over", "Over",
                                              gsub("less", "Less", toTitleCase(gsub("_"," ",`V06 tsi,ses_income`))))))

summ_stats_table <-
  tbl_summary(dataset_wide_forsum_stats_v2, by=Risk_dummy, 
              include=c("Gender_Dummy", "Race", "Ethnicity", "V24_ASD_dummy", 
                        "V06 tsi,mother_education_edited", "V06 tsi,ses_income_edited"),
              label=c(Gender_Dummy~"Gender", V24_ASD_dummy~"24 Month Diagnosis",
                      `V06 tsi,mother_education_edited`="Maternal Education",
                      `V06 tsi,ses_income_edited`="Household Income")) %>% 
  add_p(test=list(`V06 tsi,ses_income_edited` ~ "chisq.test")) %>%
  #add_stat_label() %>%
  as_flex_table() %>%
  autofit()
...
# Finally, get number of obs. per group with final counts
ll_group_final_ns <- dataset_wide_forsum_stats_v2 %>%
  filter(Risk_dummy=="LR") %>%
  select(Identifiers) %>%
  unlist()

ll_ns_test <- dataset_long %>% 
  filter(Identifiers %in% ll_group_final_ns) %>%
  filter(is.na(aosi_total_score_1_18)==0|is.na(ADOS_Comb_EC)==0) %>%
  select(Identifiers, V24_Group, Visit, aosi_total_score_1_18, ADOS_Comb_EC)

ll_ns_test_2 <- dataset_long %>% 
  filter(Identifiers %in% ll_group_final_ns) %>%
  select(Identifiers, V24_Group, Visit, aosi_total_score_1_18, ADOS_Comb_EC)

# Get obs from ll_ns_test
ll_ns_test %>% select(Identifiers) %>% unlist() %>% unique() %>% length()
ll_ns_test %>% dim()
...
venn.diagram(
  ...
  filename = "Plots_Tables/male_venn_diagram_size.jpg",
  output=TRUE,
...
venn.diagram(
  x = list(asd, hssc_sc, mirc_rrb),
  category.names = c("ASD DX" , "HSSC Group (SC) " , "MIRC Group (RRB)"),
  filename = "Plots_Tables/female_venn_diagram_adjust.jpg",
  output=TRUE,
  col=c("#440154ff", '#21908dff', 'orange'),
  fill = c(alpha("#440154ff",0.3), alpha('#21908dff',0.3), alpha('orange',0.3)),
  cex = 0.5,
...
### 9. Examine rates of 1) ASD and 2) no. of visits by group
## RRB
# Load group data
long_data_rrb <- read_csv("Data/x_var_data_rrb.csv")
dataset_num_ids <- read_csv(file="Docs/num_ids_og_ids_key.csv") %>% plyr::rename(replace=c("num_ids"="ID"))
load("traj_results/rrb/gmm_predprob.RData")

# Count no of obs per person
num_per_person <- long_data_rrb %>% 
  group_by(ID, DXGENCHA) %>%
  summarise(n=n())

# Merge all data together
rrb_cluster_n_data <- left_join(gmm_predprob, dataset_num_ids) %>%
  left_join(num_per_person) %>%
  mutate(v24_asd = ifelse(grepl("ASD", DXGENCHA), "ASD", 
                          ifelse(grepl("negative", DXGENCHA), "Negative", NA)),
         gen_char = ifelse(grepl("female", DXGENCHA), "female", 
                           ifelse(grepl("male", DXGENCHA), "male", NA)),
         n_cat = factor(n),
         class_cat = factor(class))

# Test if number of points associated with cluster membership
prop.table(table(rrb_cluster_n_data$class_cat, rrb_cluster_n_data$n_cat), margin = 1) %>%
  round(2)
fisher.test(table(rrb_cluster_n_data$class_cat, rrb_cluster_n_data$n_cat))

# Test if ASD associated with cluster membership
prop.table(table(rrb_cluster_n_data$class_cat, rrb_cluster_n_data$v24_asd), margin = 1)
fisher.test(table(rrb_cluster_n_data$class_cat, rrb_cluster_n_data$v24_asd))
...
## SC
# Load group data
long_data_soc <- read_csv("Data/x_var_data_soc.csv")
dataset_num_ids <- read_csv(file="Docs/num_ids_og_ids_key.csv") %>% plyr::rename(replace=c("num_ids"="ID"))
load("traj_results/soc/gmm_predprob.RData")

# Count no of obs per person
num_per_person <- long_data_soc %>% 
  group_by(ID, DXGENCHA) %>%
  summarise(n=n())

# Merge all data together
soc_cluster_n_data <- left_join(gmm_predprob, dataset_num_ids) %>%
  left_join(num_per_person) %>%
  mutate(v24_asd = ifelse(grepl("ASD", DXGENCHA), "ASD", 
                          ifelse(grepl("negative", DXGENCHA), "Negative", NA)),
         n_cat = factor(n),
...
# Test if number of points associated with cluster membership
prop.table(table(soc_cluster_n_data$class_cat, soc_cluster_n_data$n_cat), margin = 1)
fisher.test(table(soc_cluster_n_data$class_cat, soc_cluster_n_data$n_cat))

# Test if ASD associated with cluster membership
prop.table(table(soc_cluster_n_data$class_cat, soc_cluster_n_data$v24_asd), margin = 1)
fisher.test(table(soc_cluster_n_data$class_cat, soc_cluster_n_data$v24_asd))

#Reset here
rm(list=ls())

### 10. Check if maternal education is associated with cluster/factor score controlling for others (age, sex, diagnosis)
## RRB
# Load data
dataset_num_ids <- read_csv(file="Docs/num_ids_og_ids_key.csv") %>% plyr::rename(replace=c("num_ids"="ID"))
load("traj_results/rrb/gmm_predprob.RData")

added_demo_info <- read_csv("Data/TSI_V06.csv") %>% select(Identifiers, `V06 tsi,mother_education`) %>% 
  full_join(dataset_num_ids)

gmm_data_with_demo <- left_join(gmm_predprob, added_demo_info)

ids_missing_mat_edu <- gmm_data_with_demo %>% 
  filter(is.na(`V06 tsi,mother_education`)==1) %>% 
  select(Identifiers) %>% unlist()
added_demo_info_check <- added_demo_info %>% filter(Identifiers%in%ids_missing_mat_edu)

# Check cluster prob using logistic regression
added_vars_rrb <- read_csv("Data/x_var_data_rrb.csv") %>% 
  select(ID, GCHAR, DXGENCHA) %>%
  mutate(v24_asd = ifelse(grepl("ASD", DXGENCHA), "ASD", 
                          ifelse(grepl("negative", DXGENCHA), "Negative", NA)))
rrb_data_glm <- left_join(gmm_data_with_demo, added_vars_rrb) %>%
  mutate(class_fct = factor(class)) %>%
  distinct()

rrb_cluster_mat_edu_glm_fit <- glm(class_fct~GCHAR+v24_asd+`V06 tsi,mother_education`, 
    data=rrb_data_glm, family=binomial())
sink("Plots_Tables/rrb_cluster_mat_edu_glm_fit.txt")
summary(rrb_cluster_mat_edu_glm_fit)
sink()

# Also check missing data probability (some observed or not)
long_data_rrb <- read_csv("Data/x_var_data_rrb.csv")
num_per_person <- long_data_rrb %>% 
  group_by(ID, DXGENCHA) %>%
  summarise(n=n())

rrb_data_glm_v2 <- left_join(rrb_data_glm, num_per_person) %>%
  mutate(missing_obs = factor(ifelse(n<4, 1, 0)))

rrb_missingobs_mat_edu_glm_fit <- glm(missing_obs~GCHAR+v24_asd+`V06 tsi,mother_education`, 
                                   data=rrb_data_glm_v2, family=binomial())

sink("Plots_Tables/rrb_missingobs_mat_edu_glm_fit.txt")
summary(rrb_missingobs_mat_edu_glm_fit)
sink()
...
# Check cluster prob using logistic regression
added_vars_soc <- read_csv("Data/x_var_data_soc.csv") %>% 
  select(ID, GCHAR, DXGENCHA) %>%
  mutate(v24_asd = ifelse(grepl("ASD", DXGENCHA), "ASD", 
                          ifelse(grepl("negative", DXGENCHA), "Negative", NA)))
soc_data_glm <- left_join(gmm_data_with_demo, added_vars_soc) %>%
  mutate(class_fct = factor(class)) %>%
  distinct()

soc_cluster_mat_edu_glm_fit <- glm(class_fct~GCHAR+v24_asd+`V06 tsi,mother_education`, 
                                   data=soc_data_glm, family=binomial())

sink("Plots_Tables/soc_cluster_mat_edu_glm_fit.txt")
summary(soc_cluster_mat_edu_glm_fit)
sink()
...
soc_data_glm_v2 <- left_join(soc_data_glm, num_per_person) %>%
  mutate(missing_obs = factor(ifelse(n<4, 1, 0)))

soc_missingobs_mat_edu_glm_fit <- glm(missing_obs~GCHAR+v24_asd+`V06 tsi,mother_education`, 
                                      data=soc_data_glm_v2, family=binomial())

sink("Plots_Tables/soc_missingobs_mat_edu_glm_fit.txt")
summary(soc_missingobs_mat_edu_glm_fit)
sink()
