...
#RRB Scores
############
factor_data <- read_csv(file="../cfa_results/rrb_files/cfa_scores_wdx.csv", na=c("*","","NA","N/A","--"))

factor_data <- 
  factor_data %>%
  as.data.frame()

factor_data$ETA_Z <- scale(factor_data$rrbcfa)
...
gmm_pred_outcome <- gmm2_si.2p$pred
gmm_data <- gmm2_si.2p$data
#Input data and pred outcome data differ by 1 row.  Need to figure out why
