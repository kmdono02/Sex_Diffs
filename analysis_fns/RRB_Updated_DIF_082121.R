...
ob <-aMNLFA.object(dir          = homedir, # location of data
                   mrdata        = x_var_data_rrb, #read in dataframe from R
                   indicators    = # list a set of indicators of a single factor; make names as short as possible
                     items_rrb,
                   catindicators = c("MOTOR", "SENS", "QREPB", "Q19", "Q16"),       
...
#source("../../../aMNFLA_edit_fns/aMNLFA_itemplots.R") 
...
save(prune.object.save, file="rrb_prune_object.RData")
...
names(scores) <- c("MOTOR", "SENS", "QREPB", "Q19", "Q16", "AGE", "GNUM", "ETA", "ETA_SE", "ID")
scores_edit <- scores %>% select(ID, everything())
write_csv(x=scores_edit, file="rrb_data_w_scores.csv")

# Need to add in DX
scores_merge <- left_join(scores_edit, x_var_data_rrb %>% 
