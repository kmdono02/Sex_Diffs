aMNLFA.simultaneous<-function(input.object){
  ...
  ####ROUND 1 USES p<.1 AS GATE TO GET TO ROUND 2 FOR MEAS INV and p<.1 for IMPACT####################
  ...
  if(length(start_delete_index)>0&length(end_delete_index)>0){
    meanimpact_edit <- data.frame(meanimpact_v0[-(start_delete_index:(end_delete_index[length(end_delete_index)]-1)),])
    colnames(meanimpact_edit) <- colnames(meanimpact_v0)
    write.table(meanimpact_edit,file.path(dir,"meanimpactscript.out",sep=""),append=F,row.names=FALSE,col.names=TRUE,quote=FALSE)
  }else{
    message("RESULTS IN PROBABILITY SCALE or BRANT WALD TEST FOR PROPORTIONAL ODDS not found, continuing as normal")
  }
  
  meanimpact<-MplusAutomation::readModels(target=file.path(dir,"meanimpactscript.out",sep=""))
  meanimpact<-as.data.frame(meanimpact$parameters$unstandardized)
  meanimpact<-meanimpact[which(meanimpact$paramHeader=="ETA.ON"),]
  meanimpact_sign_var<-meanimpact[which(meanimpact$pval<.1),]
  # meanimpact_keep_apriori <- which(meanimpact$param%in%c("GNUM","DXV24","AGE",
  #                                                        "DX_G1", "DX_G2", "DX_G3"))
  # if(length(which(meanimpact$pval<.1))>0){
  #   meanimpact <- 
  #     meanimpact[sort(unique(c(meanimpact_keep_apriori,meanimpact_sign_var))),]
  # }else{
  #   meanimpact <- 
  #     meanimpact[sort(unique(c(meanimpact_keep_apriori))),]
  # }
  meanimpact <- meanimpact_sign_var
  ...
  varimpact_v0 <- read.delim(file.path(dir,"varimpactscript.out",sep=""), blank.lines.skip=FALSE)
  start_delete_index <- which(grepl("RESULTS IN PROBABILITY SCALE", unlist(varimpact_v0)))
  end_delete_index <- which(grepl("BRANT WALD TEST FOR PROPORTIONAL ODDS", unlist(varimpact_v0)))
  if(length(start_delete_index)>0&length(end_delete_index)>0){
    varimpact_edit <- data.frame(varimpact_v0[-(start_delete_index:(end_delete_index-1)),])
    colnames(varimpact_edit) <- colnames(varimpact_v0)
    write.table(varimpact_edit,file.path(dir,"varimpactscript.out",sep=""),append=F,row.names=FALSE,col.names=TRUE,quote=FALSE)
  }else{
    message("RESULTS IN PROBABILITY SCALE or BRANT WALD TEST FOR PROPORTIONAL ODDS not found, continuing as normal")
  }

  varimpact<-MplusAutomation::readModels(target=file.path(dir,"varimpactscript.out",sep=""))
  ...
  for (i in 1:length(myindicators)){
    # Remove excess output else creates error
    dif_v0 <- read.delim(file.path(dir,paste("measinvarscript_",myindicators[i],".out",sep="")), blank.lines.skip=FALSE)
    start_delete_index <- which(grepl("RESULTS IN PROBABILITY SCALE", unlist(dif_v0)))
    end_delete_index <- which(grepl("BRANT WALD TEST FOR PROPORTIONAL ODDS", unlist(dif_v0)))
    if(length(start_delete_index)>0&length(end_delete_index)>0){
    dif_edit <- data.frame(dif_v0[-(start_delete_index:(end_delete_index[length(end_delete_index)]-1)),])
    colnames(dif_edit) <- colnames(dif_v0)
    write.table(dif_edit,file.path(dir,paste("measinvarscript_",myindicators[i],".out",sep="")),
                append=F,row.names=FALSE,col.names=TRUE,quote=FALSE)
    }else{
      message("RESULTS IN PROBABILITY SCALE or BRANT WALD TEST FOR PROPORTIONAL ODDS not found, continuing as normal")
    }
    
    dif<-MplusAutomation::readModels(file.path(dir,paste("measinvarscript_",myindicators[i],".out",sep="")))
    ...
    for (j in 1:length(myMeasInvar)){
      alllambdadf[i,j+1]<-ifelse(alllambdadf[i,j+1]<.1,myMeasInvar[j],NA)
    }
  ...
  useround2<-c(diflist,keepmeanimpact,keepvarimpact,mytime)
  ...
  round2input[11,1]<-ifelse(length(myauxiliary)>0,AUXILIARY,"!")
