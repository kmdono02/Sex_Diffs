...
aMNLFA.scoreplots<-function(input.object){
  ...
  factorscores <- data.frame(lapply(factorscores, as.numeric))
  keep<-c(myID,"ETA")
  
  # Merge factor scores and data
  if(is.null(mytime)==1){
    data_plus_scores <- full_join(mrdata, factorscores %>% 
                                    select(myID, "ETA", "ETA_SE"),
                                  by=myID)
  }else{
    data_plus_scores <- full_join(mrdata, factorscores %>% 
                                    select(myID, mytime, "ETA", "ETA_SE"),
                                  by=c(myID, mytime))  
  }
  sc<-data_plus_scores
