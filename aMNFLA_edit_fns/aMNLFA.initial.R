aMNLFA.initial<-function(input.object){
  ...
  meaninput[11,1]<-ifelse(length(myauxiliary)>0,AUXILIARY,"!")
  ...
  varinput[11,1]<-ifelse(length(myauxiliary)>0,AUXILIARY,"!")
  ...
  if (thresholds==TRUE) {
    if (identical(mycatindicators, NA)|identical(mycatindicators, NULL)) {
      ...
      for (w in 1:indlength) {
        ...
        miinput[10, 1] <- usemi
        miinput[11, 1] <- ifelse(length(myauxiliary)>0,AUXILIARY,"!")
